...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/nturgbd-cross-view/test_joint.yaml',
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='the interval for storing models (#iteration)') #保存模型的间隔
    parser.add_argument(
        '--eval-interval',
        type=int,
        default=5,
        help='the interval for evaluating models (#iteration)') #评估模型的间隔
    parser.add_argument(
        '--print-log',
        type=str2bool,
        default=True,
    ...
    parser.add_argument(
        '--weights',
        default=None,
        help='the weights for network initialization')  #添加训练好的权重
    parser.add_argument(
        '--ignore-weights',
        type=str,
        default=[],
    ...
    parser.add_argument(
        ...
        default=[20, 40, 60],
        nargs='+',
        help='the epoch where optimizer reduce the learning rate') #学习率衰减
    parser.add_argument(
        '--device',
        type=int,
        default=[0],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0005,
        help='weight decay for optimizer') #优化器的权重衰减
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    data = Variable(
                        ...
                        volatile=True)
                    label = Variable(
                        label.long().cuda(self.output_device), # for test B
                        requires_grad=False,
                        volatile=True)
                    output = self.model(data)
                    if isinstance(output, tuple):
                        ...
                        l1 = 0
                    loss = self.loss(output, label) # for test B
                    score_frag.append(output.data.cpu().numpy())
                    loss_value.append(loss.data.item()) # for test B
                ...
                if wrong_file is not None or result_file is not None:
                    ...
                    true = list(label.data.cpu().numpy()) # for test B
